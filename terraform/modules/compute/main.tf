...
resource "aws_key_pair" "main" {
  ...
  public_key = file(var.public_key_path)  # Path to your ~/.ssh/id_rsa.pub
}
      ...
      <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .badge{
          ...
          color:var(--green);font-size:.7rem;font-weight:500;
        ...
        .card{
          ...
          backdrop-filter:blur(10px);transition:border-color .3s,transform .2s;
        }
    ...
    <body>
      <div class="wrap">
        <div class="hdr">
          ...
          <p class="sub">
            <span class="tag tag-blue">${var.environment}</span>
        ...
        <div class="grid">
          ...
          <div class="card">
            <div class="lbl">Instance Type</div>
            <div class="val">$INSTANCE_TYPE</div>
          </div>
          <div class="card">
          ...
          <div class="card">
            <div class="lbl">Private IP</div>
            <div class="val">$PRIVATE_IP</div>
          </div>
          <div class="card">
          ...
          <div class="card">
            <div class="lbl">Boot Time</div>
            <div class="val">$BOOT_TIME</div>
          </div>
        ...
        <div class="ft">
          Managed by <a href="#">Terraform</a> &bull; ${var.project_name} &bull; ${var.environment}
        </div>
      </div>
      <script>
        function tick(){
...
resource "aws_instance" "webserver" {
  count = var.instance_count  # One per AZ for HA
  ...
  associate_public_ip_address = true  # Public subnet — public IP

  user_data = base64encode(local.user_data)

  # Root volume — encrypted, gp2 for free tier (30 GB free)
  root_block_device {
    volume_type           = "gp2"   # FREE TIER: 30 GB of gp2 storage
    volume_size           = 10      # Reduced: 2×10 + 8 (bastion) = 28 GB < 30 GB free tier
    encrypted             = true
    delete_on_termination = true
  }

  # Basic monitoring only — detailed monitoring costs $3.50/month/instance
  monitoring = false  # FREE TIER: basic monitoring (5-min intervals) is free

  # IMDSv2 only — prevents SSRF-based metadata attacks
  metadata_options {
  ...
  tags = {
    Name = "${var.project_name}-webserver-${var.availability_zones[count.index]}"
  }
}
...
resource "aws_instance" "bastion" {
  ...
  key_name                    = aws_key_pair.main.key_name
  subnet_id                   = var.public_subnet_ids[0]  # Public subnet
  vpc_security_group_ids      = [var.bastion_sg_id]
  associate_public_ip_address = true  # Needs public IP for SSH entry
